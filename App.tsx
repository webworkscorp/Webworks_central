...
const DataEntryPanel = ({ isOpen, onClose, dateLabel, currentData, currentClients, onSave }: any) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setClientDetails(currentClients.length > 0 ? currentClients : []);
    }
  ...
  const [state, setState] = useState<BusinessState>(() => {
    ...
    if (saved) {
      try {
        ...
        return { 
          ...parsed, 
          activeRange: { ...parsed.activeRange, start: new Date(parsed.activeRange.start), end: new Date(parsed.activeRange.end) },
          currentTab: parsed.currentTab === 'finance' ? 'clients' : parsed.currentTab // Migración de tab eliminada
        };
      ...
      <header className="glass border-b border-slate-100 px-6 pt-10 pb-6 sticky top-0 z-[100]">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm overflow-hidden border border-slate-50">
        ...
        {state.currentTab === 'clients' && (
          <div className="space-y-6 fade-in-fast">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Historial de Cierres</p>
                <span className="text-[8px] font-bold text-slate-400 uppercase">{metrics.filteredClients.length} Operaciones</span>
              </div>
              
              {metrics.filteredClients.length === 0 ? (
                <EmptyState title="Vacio" description="No hay transacciones registradas." icon={CreditCard} />
              ) : (
                <div className="space-y-2">
          ...
          <div className="space-y-8 fade-in-fast">
             ...
             {metrics.daysWithData === 0 && metrics.filteredClients.length === 0 ? (
              ...
              <div className="bg-slate-900 p-8 rounded-3xl shadow-xl text-center premium-entrance">
                ...
                <h3 className="text-sm font-bold text-white mb-2 uppercase">Diagnóstico Gemini 3</h3>
                ...
                {getDaysInMonth(state.viewingMonth, YEAR_ACTIVE).map(day => {
                  ...
                  return (
                    <button 
                      ...
                      className={`group relative py-3.5 flex flex-col items-center rounded-lg transition-all active:scale-90 ${isSelected ? 'bg-slate-900 text-white' : hasData ? 'text-sky-600 bg-sky-50/50' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto glass border-t border-slate-100 px-8 pt-4 pb-8 flex justify-between items-center z-[150] safe-area-bottom">
        {NAVIGATION_TABS.map(tab => {
          ...
          return (
            ...
            >
              <div className={`p-2 rounded-xl transition-all duration-200 ${isActive ? 'bg-sky-600 text-white shadow-md' : 'text-slate-900'}`}>{tab.icon}</div>
